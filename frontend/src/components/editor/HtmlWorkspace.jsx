...
// - View: real document in a sandboxed iframe with allow-scripts (JS runs) but
...
const REPORTER = `<script>(function(){function r(){try{parent.postMessage({__studioHeight:Math.max(document.documentElement.scrollHeight,document.body?document.body.scrollHeight:0)},'*')}catch(e){}}addEventListener('load',r);[120,400,1000,2000].forEach(function(t){setTimeout(r,t)});try{new ResizeObserver(r).observe(document.documentElement)}catch(e){}})();</script>`
...
function withReporter(html) {
  ...
  if (/<\/body>/i.test(out)) return out.replace(/<\/body>/i, REPORTER + '</body>')
  return out + REPORTER
}
...
function HtmlWorkspace({ html, onCommit }, ref) {
  ...
  useEffect(() => {
    function onMsg(e) {
      const h = e.data && e.data.__studioHeight
      if (typeof h === 'number' && h > 0) setPageHeight(Math.min(60000, Math.ceil(h)))
    }
  ...
  function onIframeLoad() {
    ...
    try {
      ...
      doc.designMode = 'on'
      setPageHeight(doc.documentElement.scrollHeight)
    } catch {
      /* ignore */
    }
  }
  ...
  const scale = Math.min(1, (stage.w - 24) / contentW || 1)
  const naturalH = pageHeight || Math.max(stage.h - 24, 600)

  const srcDoc = mode === 'view' ? withReporter(compat ? withCompat(html) : html) : editSeed
  const sandbox = mode === 'view' ? 'allow-scripts allow-forms allow-popups allow-modals' : 'allow-same-origin'
    ...
    <div className="flex min-h-0 flex-1">
      <aside className="flex w-56 shrink-0 flex-col border-r border-[#e1dfdd] bg-[#faf9f8]">
        ...
        <div className="mt-3 px-3 text-xs leading-relaxed text-[#605e5c]">
          This project is stored as one HTML document. CSS, JS, and images uploaded with it are inlined into the document.
        </div>
      </aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex flex-wrap items-center gap-2 border-b border-[#e1dfdd] bg-white px-4 py-1.5">
          ...
          <span className="ml-auto text-xs text-[#605e5c]">
            {mode === 'view'
              ? 'JavaScript is running (isolated)'
              : mode === 'source'
        ...
        {mode === 'source' ? (
          ...
          <main
            ref={stageRef}
            className="relative flex flex-1 items-start justify-center overflow-auto bg-[#f3f2f1] p-3"
          >
            <div style={{ width: Math.round(contentW * scale), height: Math.round(naturalH * scale) }}>
              <div
                className="bg-white"
                style={{
                  width: contentW,
