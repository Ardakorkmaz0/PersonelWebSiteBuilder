...
// Press-and-hold, without stealing an ordinary click or a drag.
//
// The gesture has to survive a hand that is not perfectly still, and must lose
// to any real intent: move past a few pixels and it is a drag, lift early and
// it is a click. Returns the handlers to bind and a cancel for cleanup.
export const SPOTLIGHT_HOLD_MS = 550
export const SPOTLIGHT_HOLD_SLOP = 8

export function createHoldGesture(onHold, { delay = SPOTLIGHT_HOLD_MS, slop = SPOTLIGHT_HOLD_SLOP } = {}) {
  let timer = 0
  let origin = null
  ...
  return {
    cancel,
    fired: () => timer === 0 && origin === null,
    start(event) {
      cancel()
      // Secondary buttons open menus; they are not a hold.
